...
# Cours n°1- Les données en statistique<br /> <br />
### Florian Bayer
...
knitr::include_graphics("./assets/images/2-Stat/Quali.png")
```
]
...
knitr::include_graphics("./assets/images/2-Stat/Quanti.png")
```
]]
...
.pull-left3[.font80[« L’objet de la méthode statistique est la réduction des données. Une masse de données doit être remplacée par un petit nombre de quantités représentant correctement cette masse, et contenant autant que possible la totalité de l’information pertinente contenue dans les données d’origine. »  - Sir Ronald Aymler Fisher]]

.pull-right1[
```{r echo=FALSE, out.width="70%"}
knitr::include_graphics("./assets/images/2-Stat/Fisher.jpg")
```
]

<br> <br> <br> <br> <br> <br>

<b>Les statistiques</b> : 
...
knitr::include_graphics("./assets/images/2-Stat/Europe.png")
```
]
...
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./assets/images/2-Stat/Solvay_conference_1927.jpg")
```
]]
...
df.Europe <- World  %>% dplyr::select( "iso_a3","name","continent","area","pop_est")%>%
  ...
  slice(1:5) %>%  rename(Nom = name, Superficie = area, Population = pop_est, Continent = continent) 
...
knitr::include_graphics("./assets/images/2-Stat/Marie_Curie_c1920.jpg")
```
]]
...
df <- data.frame(Nom = " ", Prénom = " ", Sexe = " ", Adresse = " ")
...
```{r echo=FALSE, out.width="100%"}
df <- data.frame(Nom = "Curie", Prénom = "Marie",  Sexe = "F", adresse = "36 quai de Béthune, 75004 Paris")
...
.zoom[ .center-img[
```{r echo=FALSE, out.width="90%"}
knitr::include_graphics("./assets/images/2-Stat/Caractere.png")
```
]]
